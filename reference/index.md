# Package index

## Tabulation

- [`tab()`](https://giobuzala.github.io/surveyorso/reference/tab.md) :

  **Create a table or crosstab**

- [`tab_all()`](https://giobuzala.github.io/surveyorso/reference/tab_all.md)
  :

  **Apply
  [`tab()`](https://giobuzala.github.io/surveyorso/reference/tab.md) to
  multiple variables**

- [`tab_mr()`](https://giobuzala.github.io/surveyorso/reference/tab_mr.md)
  :

  **Create a table or crosstab for a multiple response (pick-any) set**

- [`tab_mr_all()`](https://giobuzala.github.io/surveyorso/reference/tab_mr_all.md)
  :

  **Apply
  [`tab_mr()`](https://giobuzala.github.io/surveyorso/reference/tab_mr.md)
  to multiple multiple response sets**

- [`tab_grid()`](https://giobuzala.github.io/surveyorso/reference/tab_grid.md)
  :

  **Create a full distribution table for a grid question**

- [`tab_grid_all()`](https://giobuzala.github.io/surveyorso/reference/tab_grid_all.md)
  :

  **Apply
  [`tab_grid()`](https://giobuzala.github.io/surveyorso/reference/tab_grid.md)
  to multiple grid questions**

- [`tab_grid_t()`](https://giobuzala.github.io/surveyorso/reference/tab_grid_t.md)
  :

  **Create a top/bottom box table or crosstab for a grid question**

- [`tab_grid_t_all()`](https://giobuzala.github.io/surveyorso/reference/tab_grid_t_all.md)
  :

  **Apply
  [`tab_grid_t()`](https://giobuzala.github.io/surveyorso/reference/tab_grid_t.md)
  to multiple grid questions**

## Tracking

- [`merge_waves()`](https://giobuzala.github.io/surveyorso/reference/merge_waves.md)
  :

  **Merge two survey waves**

## Statistical

- [`standardize()`](https://giobuzala.github.io/surveyorso/reference/standardize.md)
  :

  **Standardize numeric and factor variables (z-scores)**

- [`normalize()`](https://giobuzala.github.io/surveyorso/reference/normalize.md)
  :

  **Normalize numeric and factor variables to a 0–1 scale**

- [`one_hot_encode()`](https://giobuzala.github.io/surveyorso/reference/one_hot_encode.md)
  :

  **One-hot encode categorical variables**

- [`cor_matrix()`](https://giobuzala.github.io/surveyorso/reference/cor_matrix.md)
  :

  **Compute a correlation matrix**

- [`cor_heatmap()`](https://giobuzala.github.io/surveyorso/reference/cor_heatmap.md)
  :

  **Plot a correlation heatmap**

## Segmentation

- [`kmeans_grid()`](https://giobuzala.github.io/surveyorso/reference/kmeans_grid.md)
  :

  **Run a grid of k-means clustering models**

## Data quality

- [`strline()`](https://giobuzala.github.io/surveyorso/reference/strline.md)
  :

  **Identify straightliners in grid questions**

- [`lowqual_gpt()`](https://giobuzala.github.io/surveyorso/reference/lowqual_gpt.md)
  :

  **Identify low-quality open-ended responses using the OpenAI API**

## Open-ended coding

- [`export_coding()`](https://giobuzala.github.io/surveyorso/reference/export_coding.md)
  :

  **Export a coding workbook**

- [`import_coding()`](https://giobuzala.github.io/surveyorso/reference/import_coding.md)
  :

  **Import a coding workbook and merge coded variable into the dataset**

- [`theme_gpt()`](https://giobuzala.github.io/surveyorso/reference/theme_gpt.md)
  :

  **Generate a thematic code list using the OpenAI API**

- [`code_gpt()`](https://giobuzala.github.io/surveyorso/reference/code_gpt.md)
  :

  **Code open-ended survey responses using the OpenAI API**

- [`code_gpt_to_df()`](https://giobuzala.github.io/surveyorso/reference/code_gpt_to_df.md)
  :

  **Process
  [`code_gpt()`](https://giobuzala.github.io/surveyorso/reference/code_gpt.md)
  output to the dataset**

- [`code_gpt_to_excel()`](https://giobuzala.github.io/surveyorso/reference/code_gpt_to_excel.md)
  :

  **Write
  [`code_gpt()`](https://giobuzala.github.io/surveyorso/reference/code_gpt.md)
  results to Excel workbook**

## Miscellaneous

- [`combine()`](https://giobuzala.github.io/surveyorso/reference/combine.md)
  :

  **Combine split variables into a single variable**

## S3 methods

- [`print(`*`<tab_result>`*`)`](https://giobuzala.github.io/surveyorso/reference/print.tab_result.md)
  :

  **Custom print method for
  [`tab()`](https://giobuzala.github.io/surveyorso/reference/tab.md)
  results**

- [`print(`*`<tab_mr_result>`*`)`](https://giobuzala.github.io/surveyorso/reference/print.tab_mr_result.md)
  :

  **Custom print method for
  [`tab_mr()`](https://giobuzala.github.io/surveyorso/reference/tab_mr.md)
  results**

- [`print(`*`<tab_grid_result>`*`)`](https://giobuzala.github.io/surveyorso/reference/print.tab_grid_result.md)
  :

  **Custom print method for
  [`tab_grid()`](https://giobuzala.github.io/surveyorso/reference/tab_grid.md)
  results**

- [`print(`*`<tab_grid_t_result>`*`)`](https://giobuzala.github.io/surveyorso/reference/print.tab_grid_t_result.md)
  :

  **Custom print method for
  [`tab_grid_t()`](https://giobuzala.github.io/surveyorso/reference/tab_grid_t.md)
  results**
